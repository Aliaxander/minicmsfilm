...
function clone(obj) {
    ...
    for (p in obj) {
        if (obj.hasOwnProperty(p)) {
            v = obj[p];
            if (v && typeof v === 'object') c[p] = clone(v);
            else c[p] = v
        }
    }
    return c
...
function icon(icon, iclass) {
    ...
    if (iclass) $(span).addClass(iclass);
    return span
}
...
function ajax(_url, _data) {
    ...
    opts.type = 'post';
    if (data) opts.data = isForm ? $(data).serialize() : data;
    var err = function() {
        error({
            text: 'Ошибка сервера'
        })
    ...
    var mess = function(resp) {
        if (resp && resp.error) {
            if (resp.urlReload) window.location.reload();
            else if (resp.urlRedirect) window.location.href = resp.urlRedirect;
            else if (resp.not_authorized) dialog({
    ...
    var showIndicator = function() {
        if (isForm) disableForm(data);
        $('body').addClass('wait')
    };
    var hideIndicator = function(response) {
        if (isForm) enableForm(data);
    ...
    show: function() {
        ...
        if (self.options.buttons) {
            ...
            for (var i in self.options.buttons) {
                ...
                if (self.options.buttons[i]['class']) $(button).addClass(self.options.buttons[i]['class']);
                $(button).bind('click', function() {
                    if (self.options.buttons[$(this).attr('button-id')]['callback']) self.options.buttons[$(this).attr('button-id')]['callback'](self)
                })
            }
        }
        ...
        var mousewheel = function(e, delta) {
            if (0) {
                if (delta > 0) $('.text', div)[0].scrollTop -= 20;
                else $('.text', div)[0].scrollTop += 20;
                return false
            }
        };
...
function form_dialog(options) {
    if (disable_form_dialog) return false;
        ...
        if (data && !data.error) {
            ...
            $(form.cont).find('form').bind('submit', function() {
                ajax($(this)).success(function(data) {
                    if (options.callback(data) && data.success) form.destroy()
                });
                return false
            });
    ...
    videoFlash.prototype.support = function(format) {
        ...
        if (!version || version < this.minFlashVersion) return false;
        for (var k = 0; k < this.supportedFormats.length; k++)
            if (this.supportedFormats[k] == format) return true;
        return false
    };
    ...
    var videoPlayerFlash = window['videoPlayerFlash'] = {
        ...
        ready: function(swfID) {
            ...
            if (player && player.video && player.video.event) player.video.event('ready')
        },
        event: function(swfID, eventName) {
            ...
            for (var i = 0; i < events.length; i++) {
                if (events[i] == eventName) {
                    var playerId = this.parseId(swfID);
                    var player = players.get(playerId);
                    if (player && player.video && player.video.event) player.video.event(eventName)
                }
            }
        },
    ...
    videoFlash.prototype.property = function(propName, value) {
        ...
        if (typeof value != 'undefined') this.htmlElement.el.vjs_setProperty(propName, value);
    ...
    controls.prototype.hideOnMosemove = function(player) {
        ...
        var hide = function() {
            ...
            if (!player.paused()) timeout = setTimeout(function() {
...
mAuth = {
    ...
    init: function(params) {
        var self = this;
        self.params = params;
        var auth = $('#header').find('.auth');
        if (params.user_ok) {
            auth.find('.exit').show()
        } else {
            auth.find('.enter').show()
        }
        ...
        $('.popup form').livequery('submit', function() {
            var form = $(this);
            ajax($(this)).success(function(data) {
                if (data.form_errors) self.form_errors(form, data.form_errors);
                if (data.content) {
...
mFilm = {
    ...
    init: function() {
        ...
        if (typeof this.options.video == 'undefined') return false;
        if (typeof this.options.play_preview == 'undefined') return false;
        if (typeof mAuth.params.user_ok == 'undefined') return false;
        this.initPlayer();
        $('form.write-comment').livequery('submit', function() {
            if (!mAuth.params.user_ok) mAuth.block();
            $(this).find(':input').val('');
            return false
        })
    },
    initPlayer: function() {
        ...
        self.player.on('play', function() {
            $('.film .head').addClass('playing').removeClass('preview')
        });
        if (user_ok) return true;
        self.player.on('download', function() {
            self.player.stop();
            self.player.exitFullScreen();
            $('.film .head').removeClass('playing').addClass('preview');
            mAuth.block();
            return false
        });
        ...
        $('.controls-quality', $(player)).click(function() {
    ...
    initInfoTable: function() {
        $(function() {
            ...
            var show_all = $('.show-all', table.parent());
            if ($('.row', table).length <= 5) {
                show_all.remove();
                return false
            }
            show_all.bind('click', function() {
                if (table.hasClass('all')) {
                    var scroll_top = $(document).scrollTop();
                    var link_offset_top = show_all.offset().top;
                    table.removeClass('all');
                    show_all.text('Показать все о фильме');
                    $(document).scrollTop(show_all.offset().top - (link_offset_top - scroll_top))
                } else {
                    table.addClass('all');
    ...
    background: function(background_url) {
        if (screen.width > 850 || screen.height > 850) $('#page').css('background-image', 'url("' + background_url + '")').addClass('background-image')
    },
    _agent: null,
    agent: function() {
...
$(function() {
    var loading = false;
    $('.catalog .show-more a.load').livequery('click', function() {
...
$(function() {
    ...
    $('.show-hide-menu, .menu-shadow').click(function() {
...
$(document).ready(function() {
    var sheet = null;
    var i = 0;
    for (i = 0; document.styleSheets.length; i++) {
        var s = document.styleSheets[i];
        if ((s.id && s.id == 'page-height') || (s.ownerNode && s.ownerNode.id && s.ownerNode.id == 'page-height')) {
            sheet = s;
            break
        }
    }
    if (sheet) {
        ...
        if (rules) {
            var calcSize = function() {
                ...
                var selectors = ['body', '#page', '#background-color', '#background-dots'];
                for (i = 0; i < selectors.length; i++) {
                    if (sheet.insertRule) sheet.insertRule(selectors[i] + '{min-height:' + $(window).height() + 'px;}', 0);
                    else if (sheet.addRule) sheet.addRule(selectors[i], 'min-height:' + $(window).height() + 'px;', 0)
                }
            };
...
$(function() {
    var scrollable = function(element) {
        ...
        self.scroll_bar = this.element.next();
        self.scroll = self.scroll_bar.find('.scroll');
        self.draw();
    ...
    scrollable.prototype = {
        draw: function() {
            var width = this.element.prop('clientWidth') / this.element.prop('scrollWidth') * 100;
            ...
            this.scroll.css('width', width + '%');
            var left = this.element.prop('scrollLeft') / (this.element.prop('scrollWidth') - this.element.prop('clientWidth')) * (100 - width);
            if (100 - left < width) left = 100 - width;
            this.scroll.css('left', left + '%')
        },
        setValue: function(value) {
            var self = this;
            self.element.prop('scrollLeft', value);
            self.draw();
            return true
        },
        scrollTo: function(el) {
            var self = this;
            self.setValue($(el).prop('offsetLeft') - Math.round((self.element.prop('clientWidth') - $(el).prop('clientWidth')) * 0.5))
        },
        wheel: function(value) {
            var self = this;
            var max_left = self.element.prop('scrollWidth') - self.element.prop('clientWidth');
            var left = self.element.prop('scrollLeft');
            if (value == 0) return false;
            if (value < 0 && left == 0) return false;
            if (value > 0 && max_left == left) return false;
            self.element.prop('scrollLeft', left + value);
            self.draw();
            return true
        },
        onmousewheel: function() {
            var self = this;
            self.element.mousewheel(function(e, delta) {
                return !self.wheel(delta * (-1) * 50)
            })
        },
        onscrollmovie: function() {
            var self = this;
            var mouseXY = {
                scrolling: false,
                x: 0,
            ...
            var scrollMove = function(e) {
                ...
                var clientWidth = self.element.prop('clientWidth');
                var scrollLeft = self.element.prop('scrollLeft');
                var scrollWidth = self.element.prop('scrollWidth');
                var delta = deltaX / (self.scroll_bar.prop('clientWidth') - self.scroll.prop('clientWidth'));
                delta = Math.round(delta * (scrollWidth - clientWidth));
                if (self.wheel(delta)) {
                    mouseXY.x = x;
                    mouseXY.y = y
                }
            ...
            self.scroll.mousedown(scrollMoveStart);
            $(document).mouseup(scrollMoveEnd);
            $(document).mousemove(scrollMove)
        },
        ontouchwheel: function() {
            var self = this;
            var touchXY = {
                x: 0,
                y: 0,
            ...
            var touchStart = function(e) {
                ...
                touchXY.x = e.originalEvent.touches[0].pageX;
                touchXY.y = e.originalEvent.touches[0].pageY;
                touchXY.wheel = null
            };
            var touchMove = function(e) {
                ...
                if (touchXY.wheel === false) {
                    return
                } else if (touchXY.wheel === null) {
            ...
            inertialScroll = function() {
                ...
                scrollInertia.interval = setInterval(function() {
                    self.wheel((-1) * scrollInertia.scrollX);
                    ...
                    if (scrollInertia.scrollX == 0) clearInterval(scrollInertia.interval)
                }, 15)
            };
            ...
            self.element.on('touchstart', touchStart);
            self.element.on('touchmove', touchMove);
            self.element.on('touchend', touchEnd);
            self.element.on('touchleave', touchEnd);
            self.element.on('touchcancel', touchEnd)
        }
    };
    $('.episodes-list').livequery(function() {
        var cont = $(this);
        var scroll_s = new scrollable($('.seasons'));
        var scroll_e = new scrollable($('.episodes'));
        scroll_s.scrollTo($('.seasons a.selected'));
        scroll_e.scrollTo($('.episodes .episode.selected'));
        $('.seasons a').click(function() {
            if ($(this).hasClass('selected')) return false;
            $('.seasons a.selected', cont).removeClass('selected');
            $('.episodes .season.selected', cont).removeClass('selected');
            $(this).addClass('selected');
            $('.episodes .season[data-season-num=' + $(this).data('season-num') + ']', cont).addClass('selected');
            scroll_e.setValue(0);
            scroll_e.draw();
            return false
        })
...
$(function() {
    var slider = null;
    var createSlider = function(urls, open) {
        destroySlider();
        var container = $('<div/>').addClass('stills-slider');
        var wrapper = $('<div/>').addClass('slider-wrap');
        var images = [];
        container.append(wrapper);
        $(document.body).append(container);
        var loadImage = function(container, url) {
            var img = $(container).find('.img');
            if (img.css('background-image') != 'none') return;
            var tmp = new Image();
            tmp.onload = function() {
                img.css('background-image', 'url(' + tmp.src + ')')
            };
            tmp.src = url
        };
        for (var i = 0; i < urls.length; i++) {
            images[i] = $('<div/>').addClass('slider-image').append($('<div/>').addClass('img').css('background-image', 'none'));
        ...
        slider = {
            ...
            images: images,
            urls: urls,
            current_image_index: open ? open : 0,
            calc_size: function() {
                ...
                this.slideTo(this.current_image_index, false)
            },
            slideTo: function(i, animate) {
                if (this.animation) return false;
                if (!this.images[i]) return false;
                var image = this.images[i];
                var scroll = this.wrapper.scrollLeft() + image.position().left;
                if (image && this.urls[i]) loadImage(image, this.urls[i]);
                var next = i + 1;
                next = (next >= this.images.length) ? null : next;
                if (this.images[next] && this.urls[next]) loadImage(this.images[next], this.urls[next]);
                var prev = i - 1;
                prev = (prev < 0) ? null : prev;
                if (this.images[prev] && this.urls[prev]) loadImage(this.images[prev], this.urls[prev]);
                if (animate) {
                    var self = this;
                    this.animation = true;
            ...
            slideNext: function() {
                var next = this.current_image_index + 1;
                if (next >= this.images.length) return false;
                return this.slideTo(next, true)
            },
            slidePrev: function() {
                var prev = this.current_image_index - 1;
                if (prev < 0) return false;
                return this.slideTo(prev, true)
            }
        };
        slider.calc_size();
        var touchXY = null;
        ...
        var touchMove = function(e) {
            if (touchXY == null) return;
            var deltaX = touchXY.x - e.originalEvent.touches[0].pageX;
            var deltaY = touchXY.y - e.originalEvent.touches[0].pageY;
            if (Math.abs(deltaX) < 25 && Math.abs(deltaY) < 25) return;
            touchXY = null;
            if (Math.abs(deltaX) > Math.abs(deltaY)) {
                if (deltaX < 0) slider.slidePrev();
        ...
        $(container).on('touchend', touchMove);
        $(container).on('touchleave', touchMove);
        $(container).on('touchcancel', touchMove);
        return slider
    };
    $(window).resize(function() {
        if (slider) slider.calc_size()
    });
    ...
    var destroySlider = function() {
        if (!(slider && slider.container)) return false;
        slider.container.remove();
        slider = null;
        return true
    };
    $('.stills').livequery(function() {
        var stills_cont = $(this);
        var stills = stills_cont.find('.still');
        var show_all = stills_cont.find('.show-all');
        var show_stills = function(all) {
            var i = 0,
                still = null;
            var counts = [{
                max_w: 499,
            ...
            for (i = 0; i < counts.length; i++) {
                if ((w_width <= counts[i].max_w) || (counts[i].max_w === null)) {
                    count = counts[i].count;
                    break
                }
            }
            var limit = all ? stills.length : Math.min(count, stills.length);
            for (i = 0; i < limit; i++) {
                still = $(stills[i]);
                if (still.css('background-image') == 'none') still.css('background-image', 'url(\'' + still.data('prview-url') + '\')')
            }
            if (stills.length <= count) stills_cont.addClass('short');
            else stills_cont.removeClass('short')
        };
        ...
        show_all.bind('click', function() {
            var link = $(this);
            if (stills_cont.hasClass('show-all')) {
                var scroll_top = $(document).scrollTop();
                var link_offset_top = link.offset().top;
                stills_cont.removeClass('show-all');
                link.html('Показать все кадры');
                $(document).scrollTop(link.offset().top - (link_offset_top - scroll_top))
            } else {
                stills_cont.addClass('show-all');
                show_stills(true);
                link.html('Скрыть')
            }
        });
        stills_cont.find('.still').bind('click', function() {
            var images = [];
            var open = 0;
            var still = null;
            var s = $(this);
            for (var i = 0; i < stills.length; i++) {
                still = $(stills[i]);
                images.push(still.data('url'));
                if (still[0] == s[0]) open = i
            }
            createSlider(images, open)
        })
    })
});