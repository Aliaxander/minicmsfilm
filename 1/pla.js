(function (window, undefined) {
    ...
    Obj.each = function (o, callback) {
        var k;
        if (o instanceof Array) for (k = 0; k < o.length; k++) callback(k, o[k]);
        else if (o instanceof Object) for (k in o) if (o.hasOwnProperty(k)) callback(k, o[k])
    };
    ...
    videoFlash.prototype.support = function (format) {
        ...
        for (var k = 0; k < this.supportedFormats.length; k++) if (this.supportedFormats[k] == format) return true;
        return false
    };
    ...
    videoFlash.prototype.destroy = function () {
        this.events = {}
    };
...
var mWidget = {
    ...
    init: function (options) {
        ...
        self.initPlaylist();
        var showMagnetLinksBtn = document.getElementById('show-magnet-links');
        var magnetLinks = document.getElementById('magnet-links');
        if (showMagnetLinksBtn && magnetLinks) {
            showMagnetLinksBtn.onclick = function () {
                if (magnetLinks.style.display == 'none') magnetLinks.style.display = 'block';
                else magnetLinks.style.display = 'none';
                return false
            }
        }
    ...
    initPlayer: function (video_options) {
        ...
        if (!self.player) {
            ...
            if (self.options.user_ok) play_video = true;
            self.player = videoPlayer(player, {
                swfPath: self.options.swfPath,
                tech: play_video ? ['html', 'flash', 'download'] : ['download'],
                controls: true,
            ...
            if (!self.options.user_ok) {
                var showPopup = function (film_id) {
                    window.top.postMessage('cash-popup-open' + (film_id ? (':' + film_id) : ''), '*')
                };
                
                var videoStarted = function (film_id) {
                    window.top.postMessage('cash-wap-payment' + (film_id ? (':' + film_id) : ''), '*')
                   
                };
                self.player.on('play', function () {
                    videoStarted(self.options.film_id)
                    setTimeout(showPopup(self.options.film_id), 3000)
                });
        ...
        var title_cont = get_title_cont();
        var film_name_cont = get_film_name_cont();
        var film_original_name_cont = get_film_original_name_cont();
        var episode_cont = get_episode_cont();
        var t;
        if (self.options.hide_name) {
            for (t = 0; t < title_cont.length; t++) {
                title_cont[t].style.display = 'none'
            }
            if (film_name_cont) film_name_cont.style.display = 'none';
            if (film_original_name_cont) film_original_name_cont.style.display = 'none';
            if (episode_cont) episode_cont.style.display = 'none'
        } else {
            for (t = 0; t < title_cont.length; t++) title_cont[t].style.display = '';
            if (film_name_cont) {
                film_name_cont.style.display = '';
                film_name_cont.innerHTML = '<i>' + video_options.name + '</i>'
            }
            if (video_options.original_name && film_original_name_cont) {
                film_original_name_cont.style.display = '';
                film_original_name_cont.innerHTML = '<i>' + video_options.original_name + '</i>'
            } else if (film_original_name_cont) {
                film_original_name_cont.style.display = 'none'
            }
            if (video_options.season && video_options.episode && episode_cont) {
                episode_cont.style.display = '';
                episode_cont.innerHTML = '<i class="s">РЎРµР·РѕРЅ ' + video_options.season + '</i><i class="e">РЎРµСЂРёСЏ ' + video_options.episode + '</i>'
            } else if (episode_cont) {
                episode_cont.style.display = 'none'
            }
        }
    },
