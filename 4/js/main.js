...
mAuth = {
    ...
    init: function(params) {
        var self = this;
        self.params = params;
        var footer = $('#footer').find('.auth');
        if (params.user_ok) {
            var logout_link = $('<a/>').attr('data-action', 'logout').html('Выход');
            footer.append(logout_link)
        } else {
            ...
            footer.append(auth_link);
            footer.append(reg_link)
        }
        ...
        $('form.write-comment').livequery('submit', function() {
            if (!self.params.user_ok)
                self.block();
            $(this).find(':input').val('');
            return false
        });
        $('.film .play.block').livequery('click', function() {
            self.block();
            return false
        })
    },
...
$(function() {
    $('.show-search-form').bind('click', function() {
        var form = $('#search');
    ...
    $('.films-categories h2').bind('click', function() {
        var categories = $(this).parents('.films-categories');
